...
from .new_categorical_kernel import CategoricalKernel2 # Memory efficient re-implementation of the categorical kernel
# TrainingData替代类
class TrainingData:
    """TrainingData的简单替代实现"""
...
class NewMixedSingleTaskGP(SingleTaskGP):
    ...
    def __init__(
        ...
        # 获取设备（从 train_X 或使用 CPU）
        device = train_X.device if hasattr(train_X, 'device') else torch.device('cpu')
        outputscale_prior.low = outputscale_prior.low.to(device)
        outputscale_prior.high = outputscale_prior.high.to(device)
