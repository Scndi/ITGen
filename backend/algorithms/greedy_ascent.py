...
def acquisition_maximization_with_indices(cur_seqs, opt_indices, batch_size, stage, hb, surrogate_model, reference=None, dpp_type='no', acq_with_opt_indices=True):
    ...
    if dpp_type == 'no' or dpp_type == 'no_one':
        ...
        # 获取设备（从模型参数或使用 CPU）
        device = next(surrogate_model.model.parameters()).device if hasattr(surrogate_model, 'model') and next(surrogate_model.model.parameters(), None) is not None else torch.device('cpu')
        if acq_with_opt_indices:
            Lmat = surrogate_model.get_covar(global_candidates[:,opt_indices].to(device)).cpu().detach().numpy()
        else:
            Lmat = surrogate_model.get_covar(global_candidates.to(device)).cpu().detach().numpy()
