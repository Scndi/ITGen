from .fitting import fit_model_partial
...
class MyGPModel(BaseModel):
    def __init__(self, fit_iter : int = 20, device = None):
        self.model = None 
    ...
    def fit_partial(self, hb, opt_indices, init_ind, prev_indices):
        ...
        del self.model
        self.model = fit_model_partial(hb, opt_indices, init_ind, prev_indices, params=params, fit_iter=self.fit_iter, device=self.device)
    ...
    def acquisition(self, eval_X, bias=None):
        with torch.no_grad():
            eval_X_device = eval_X.to(dtype=torch.double).to(self.device)
            mean, var = self.predict(eval_X_device)
            ei = expected_improvement(mean, var, bias).cpu().detach()
        del mean, var, eval_X_device
        return ei
