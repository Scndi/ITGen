...
def get_bpe_substitues(substitutes, tokenizer, mlm_model):
    ...
    # 从 mlm_model 获取设备（自动检测，优先GPU）
    device = next(mlm_model.parameters()).device if hasattr(mlm_model, 'parameters') and next(mlm_model.parameters(), None) is not None else (torch.device('cuda') if torch.cuda.is_available() else torch.device('cpu'))
...
def get_masked_code_by_var(tokens: list, positions: dict):
    '''
    '''
    masked_token_list = []
    masked_var_list = []
...
def get_replaced_var_code_with_robustness(tokens: list, positions: dict, variable_embs, variable_names):
    '''
    '''
    size = len(variable_names)
    bins = 16
    bin_interval = int(size/16)
    robustness_vars = ["a","b","c","de", "fg","hi","jkl","mno","pqr","stuv","wxyz","abcde","fghig","klmno","pqrst","uvwxyz"]
    ...
    for variable_name in positions.keys():
        if variable_name in variable_names:
            ...
            variable_emb = variable_embs[variable_index]
            similarity = cosine_similarity(np.array(variable_embs),
                                           np.array([variable_emb]))

            inds = heapq.nlargest(size, range(len(similarity)), similarity.__getitem__)
            for i in range(16):
                replace_index = inds[i*bin_interval]
                robustness_vars[i] = variable_names[replace_index]

        for tmp_var in robustness_vars:
            tmp_tokens = copy.deepcopy(tokens)
            for pos in positions[variable_name]:
                tmp_tokens[pos] = tmp_var
...
def getTensor(batch, batchfirst=False, device=None):
    
    inputs, labels = batch['x'], batch['y']
    # 自动检测设备：优先GPU，找不到则CPU
    if device is None:
