...
class WIRAttacker(BaseAttacker):
    """
    WIR攻击算法 - 基于重要性加权的标识符替换

    根据标识符的重要性进行加权替换的对抗攻击算法。
    """

    def __init__(self, model, tokenizer, config: Dict[str, Any]):
        super().__init__(model, tokenizer, config)
        logger.info("✓ WIR攻击器初始化完成（框架）")
    ...
    def get_supported_model_types(self) -> List[str]:
        """返回支持的模型类型"""
        return ['roberta', 'codebert', 'codet5']
